...
class StGen:
    ...
    def __init__(self, rng=None, seed=None):
        ...
        if rng==None:
            self.rng = numpy.random.default_rng()    #numpy.random.RandomState()
        else:
            self.rng = rng
    ...
    def poisson_generator(self, rate, t_start=0, t_stop=1000, array=False,debug=False):
        """
        ...
        if type(n) is int:
            ...
            if type(number) is int:
            	...
            	number = numpy.ceil(n+3*numpy.sqrt(n)).astype(int)
            if number<100:
            	number = min(5+numpy.ceil(2*n),100)
            	if type(number) is int:
            		pass    
            	else: 
            		number = min(5+numpy.ceil(2*n),100).astype(int)
            if number > 0:
                isi = self.rng.exponential(1/rate, number)*1000
                if number > 1:
        ...
        if i==len(spikes):
            ...
            t_last = spikes[-1] + self.rng.exponential(1/rate, 1)[0]*1000

            while (t_last<t_stop):
                extra_spikes.append(t_last)
                t_last += self.rng.exponential(1/rate, 1)[0]*1000
    ...
    def gamma_generator(self, a, b, t_start=0, t_stop=1000, array=False,debug=False):
        """
        ...
        # less wasteful than double length method above
        n = (t_stop-t_start)/1000*(a*b)
        number = numpy.ceil(n+3*numpy.sqrt(n))
        if number<100:
            number = min(5+numpy.ceil(2*n),100)
        
        if number > 0:
            isi = self.rng.gamma(a, b, number)*1000
            if number > 1:
                spikes = numpy.add.accumulate(isi)
            else:
                spikes = isi
        ...
        if i==len(spikes):
            ...
            t_last = spikes[-1] + self.rng.gamma(a, b, 1)[0]*1000

            while (t_last<t_stop):
                extra_spikes.append(t_last)
                t_last += self.rng.gamma(a, b, 1)[0]*1000
    ...
    def _inh_gamma_generator_python(self, a, b, t, t_stop, array=False):
        ...
        while(i<len(ps)):
            ...
            if rn[i]<gamma_hazard((ps[i]-t_last)/1000,a[t_i],b[t_i])/rmax:
                # keep spike
                t_last = ps[i]
                keep[i] = True
            i+=1
    ...
    def _OU_generator_python(self, dt, tau, sigma, y0, t_start=0, t_stop=1000, array=False,time_it=False):
        """ 
    ...
    def _OU_generator_python2(self, dt, tau, sigma, y0, t_start=0, t_stop=1000, array=False,time_it=False):
        """ 
        Generates an Orstein Ulbeck process using the forward euler method. The function returns
        an AnalogSignal object.
    ...
    def OU_generator_weave1(self, dt,tau,sigma,y0,t_start=0,t_stop=1000,time_it=False):
        """ 
        Generates an Orstein Ulbeck process using the forward euler method. The function returns
        an AnalogSignal object.
...
def shotnoise_fromspikes(spike_train,q,tau,dt=0.1,t_start=None, t_stop=None,array=False, eps = 1.0e-8):
   ...
   """
    ...
    kern = q*numpy.exp(-numpy.arange(0,vs_t,dt)/tau)
