...
def createRate(d, t):
    ...
    for t in range(0,t):
        ...
        Gprime_sub2 = 36.8 + (lspot-lbkg)*(1-math.exp(-d**2/(4*a1**2))+w*(1-math.exp(-d**2/(4*a2**2))))
        ...
        Grprime_sub2 = 36.8 + (lspot-lbkg)*(((math.exp(-(ra**2)/(a1**2)))*sum1)+(0.85*((math.exp(-(ra**2)/(a2**2))*sum2))))
...
def createSubnetworkInputs(trial, parent_dir):
    ...
    for d in np.arange(0,10,0.1):
        ...
        t = 400
        d = 10 # full field
        rate_array,rate_arrayr,rate_array_sub2,rate_array2_sub2 = createRate(d,t)
        PS = createPoissonInputs(rate_array, t)
        PSr = createPoissonInputs(rate_arrayr, t)
        PS_sub2 = createPoissonInputs(rate_array_sub2,t)
        PSr_sub2 = createPoissonInputs(rate_arrayr_sub2,t)
