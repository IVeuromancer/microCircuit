...
from cell_parallel_test import IN
from cell_parallel_test import TC
import numpy as np
...
class Subnetwork:
    ...
    def set_gids(self):
        ...
        self.gidlist = list(range(pc.id(), self._N+3, pc.nhost()))
        self.gidlist = np.array(self.gidlist)+self.gidStart
        self.gidlist = np.ndarray.tolist(self.gidlist)
    ...
    def _create_cells(self, r):
        ...
        for j in range(0,len(self.gidlist)):
            if self.gidlist[j] == 0 or self.gidlist[j] == 0+self.gidStart:
                pc.cell(self.IN_1._gid, self.IN_1._spike_detector)
            if self.gidlist[j] == 1 or self.gidlist[j] == 1+self.gidStart:
                pc.cell(self.IN_1._gid2, self.IN_1._release_detector)
            if self.gidlist[j] == 2 or self.gidlist[j] == 2+self.gidStart:
                pc.cell(self.IN_1._gid3, self.IN_1._release_detector2)
            if self.gidlist[j] == 3 or self.gidlist[j] == 3+self.gidStart: 
                pc.cell(self.TCcells[0]._gid, self.TCcells[0]._spike_detector, 0)
            if self.gidlist[j] == 4 or self.gidlist[j] == 4+self.gidStart:
                pc.cell(self.TCcells[1]._gid, self.TCcells[1]._spike_detector, 0)
