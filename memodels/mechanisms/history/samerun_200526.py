...
def runSame(parent_dir, trial):
    ...
    for x in range(0,len(new_subnetwork1)): #loop through different spot diameter spike trains
        ...
        #<3 txt_ext = '.txt'
        pickle_ext = '.npy'
        folder = '../../spike_output/' + parent_dir + '/samerun_' + trial_str + '/'
        xstr = str(x)
        np.save(folder+filename_spikes+datetimestr+'-'+xstr+pickle_ext, sameIN_spike_events, allow_pickle = True)
