...
from cell_200619_heiberg import IN
from cell_200619_heiberg import TC

class Subnetwork_noaxon_final:
    """
    ...
    def __init__(self, gidStart=0, N=2, r=100):
        ...
        self.ncBr4.delay = 1
        self.nc2Br4 = h.NetCon(self._vecstim2, self.IN_1.syn_Brdist2)
        self.nc2Br4.weight[0] =0.0025
        self.nc2Br4.delay = 1
    ...
    def _add_IN_syn(self):
        ...
        #     self.nc.weight[0] = 0.0038 #0.0017 (cauases a 1mV IPSP)
        #     self.nc.delay = 0.97
        ...
        for target in self.TCcells:
            if target._gid == 5 or target._gid == self.gidStart+5:
                ...
                self.nc2.weight[0] = 0.0038 #0.0017 (causes a 1mV IPSP)
                self.nc2.delay = 0.1
            ...
            if target._gid == 6 or target._gid == self.gidStart+6:
                ...
                self.nc4.weight[0] = 0.0038 #0.0017 (causes a 1mV IPSP)
                self.nc4.delay = 0.1
            ...
            if target._gid == 7 or target._gid == self.gidStart+7:
                ...
                self.nc5.weight[0] = 0.0038 #0.0017 (causes a 1mV IPSP)
                self.nc5.delay = 0.1
            ...
            if target._gid == 8 or target._gid == self.gidStart+8:
                ...
                self.nc6.weight[0] = 0.0038 #0.0017 (causes a 1mV IPSP)
                self.nc6.delay = 0.1
    ...
    def _add_Br_Ps_syn(self):
        for target in self.TCcells:
            ...
            if target._gid == 6 or target._gid == self.gidStart+6:
                ...
                self.nc2BrsPs.append(self.nc2BrPs)
            if target._gid == 7 or target._gid == self.gidStart+7:
                self.ncBrPs = h.NetCon(self._vecstim, target.syn_Br)
                self.ncBrPs.weight[0] = 0.024 #changed from 0.003
                self.ncBrPs.delay = 1
