...
for x in range(0,len(new_subnetwork1)): #loop through different spot diameter spike trains
    ...
    all_timeseriesdata = pc.py_alltoall([samevertstack] + [None] * (pc.nhost() - 1))
    sameIN_time_series.append(all_timeseriesdata)
    
    all_spikedata = pc.py_alltoall([samevertstack2] + [None] * (pc.nhost() - 1))
    sameIN_spike_events.append(all_spikedata)
...
np.save(folder+filename_timeseries+datetimestr+pickle_ext, sameIN_time_series, allow_pickle = True)
