...
class RatSSCxMainProtocol(ephys.protocols.Protocol):
    ...
    def run(self, cell_model, param_values, sim=None, isolate=None):
        ...
        if live_plot:
            plt.plot(rmp_response['.RMP.soma.v']['time'], rmp_response['.RMP.soma.v']['voltage'])
            plt.title("RMP")
            plt.pause(0.5)
        ...
        if (rmp_score <= self.rmp_score_threshold) or (not self.use_rmp_rin_thresholds):
            ...
            if hasattr(self, 'rinhold_protocol_dep'):
                ...
                rin_dep = self.rin_efeature_dep.calculate_feature(
                rinhold_response_dep)
            ...
            if rinhold_response_hyp is not None:
                ...
                rin_hyp = self.rin_efeature_hyp.calculate_feature(
                    rinhold_response_hyp)

                rin_score = self.rin_efeature_hyp.calculate_score(
                    rinhold_response_hyp)
                ...
                if live_plot:
                    plt.plot(rinhold_response_hyp['.Rin_hyp.soma.v']['time'], rinhold_response_hyp['.Rin_hyp.soma.v']['voltage'])
                    plt.title("Rin_hyp")
                    try:
                        plt.pause(0.5)
                        plt.plot(rinhold_response_dep['.Rin_dep.soma.v']['time'], rinhold_response_dep['.Rin_dep.soma.v']['voltage'])
                        plt.title("Rin_dep")
                    except KeyError:
                        pass
...
class RatSSCxRinHoldcurrentProtocol(ephys.protocols.Protocol):
    ...
    def run(self, cell_model, param_values, sim, rmp=None):
        ...
        if "dep" in self.name:
            rin_noholding_protocol = self.create_rin_protocol_dep(
                holdi=0)
            ...
            rin_noholding_protocol = self.create_rin_protocol_hyp(
                holdi=0)
            rin_noholding_response = rin_noholding_protocol.run(
                cell_model,
                param_values,
        ...
        print("Searching holdi for vhold = {}".format(self.voltagebase_efeature.exp_mean))
        ...
        # Set up Rin protocol
        if "dep" in self.name:
            self.rin_protocol = self.create_rin_protocol_dep(
                holdi=holdi)
        if "hyp" in self.name:
            self.rin_protocol = self.create_rin_protocol_hyp(
                holdi=holdi)
    ...
    def create_rin_protocol_dep(self, holdi=None):
        ...
        for recording in rin_protocol_dep.recordings:
             recording.name = recording.name.replace(
                 ...
                 rin_protocol_dep.name)

        rin_protocol_dep.holding_stimulus.step_amplitude = \
            holdi
    ...
    def create_rin_protocol_hyp(self, holdi=None):
        ...
        for recording in rin_protocol_hyp.recordings:
             recording.name = recording.name.replace(
                 self.rin_protocol_template.name,
                 rin_protocol_hyp.name)

        rin_protocol_hyp.holding_stimulus.step_amplitude = \
            holdi
    ...
    def search_holdi(self,
        ...
        #holdi_estimate = 0.11
        print("Holdi estimate is {} with target vhold {}, rmp {}, Rin {}".format(holdi_estimate, holding_voltage, rmp, rin_noholding))
    ...
    def binsearch_holdi(self,
        ...
        if depth > max_depth:
            print("Search holdi reached max depth, returning with ihold {}".format(middle_bound))
            return middle_bound
        else:
            ...
            if middle_voltage is None:
                ...
                with open("logs/params.json", "a") as fic:
                    fic.write(json.dumps(param_values, indent = 2, sort_keys = True))
                return None
    ...
    def voltage_base(self, current, cell_model,
        ...
        if "dep" in self.name:
            ...
            if live_plot:
                import pylab
                plt.plot(
                    response[self.prefix + 'Rin_dep.soma.v']['time'],
                    response[self.prefix + 'Rin_dep.soma.v']['voltage'])
                plt.title("Holdi {} -> holdv {}".format(current, voltage_base))
                plt.pause(.1)
            ...
            if live_plot:
                import pylab
                plt.plot(
                    response[self.prefix + 'Rin_hyp.soma.v']['time'],
                    response[self.prefix + 'Rin_hyp.soma.v']['voltage'])
                plt.title("Holdi {} -> holdv {}".format(current, voltage_base))
                plt.pause(.1)
...
class RatSSCxThresholdDetectionProtocol(ephys.protocols.Protocol):
    ...
    def search_max_threshold_current(self,
        ...
        print 'Max threshold current from vhold %d: %.6g' % (self.holding_voltage, max_threshold_current)
    ...
    def create_step_protocol(self, holdi=0.0, step_current=0.0):
        ...
        if threshold_protocol.holding_stimulus is not None:
            threshold_protocol.holding_stimulus.step_amplitude = \
                holdi
        threshold_protocol.step_stimulus.step_amplitude = \
            step_current
    ...
    def create_short_threshold_protocol(
        ...
        origin_step_duration = \
            short_protocol.step_stimulus.step_duration
        origin_step_delay = \
            short_protocol.step_stimulus.step_delay
    ...
    def detect_spike(self,
        ...
        if "dep" in self.name:
            ...
            if live_plot:
                import pylab
                pylab.plot(
                    response[self.prefix + 'ThresholdDetection_dep.soma.v']['time'],
                    response[self.prefix + 'ThresholdDetection_dep.soma.v']['voltage'])
                pylab.pause(.1)
        elif "hyp" in self.name:
...
class RampThresholdProtocol(RampProtocol):
    ...
    def run(self, cell_model, param_values, sim=None, isolate=None):
        ...
        if "hyp" in self.name:
            ...
            self.ramp_stimulus.ramp_amplitude_start = \
                cell_model.threshold_current_dep * \
                (float(self.thresh_perc_start) / 100)
        ...
        responses.update(super(RampThresholdProtocol, self).run(
            cell_model,
            param_values,
            sim=sim,
            isolate=isolate))
        ...
        if live_plot:
            if "hyp" in self.name:
                ...
                stim_name = "Ramp"
            plt.plot(responses['.{}.soma.v'.format(stim_name)]['time'], responses['.{}.soma.v'.format(stim_name)]['voltage'])
            plt.title("Ramp")
            plt.pause(0.5)
